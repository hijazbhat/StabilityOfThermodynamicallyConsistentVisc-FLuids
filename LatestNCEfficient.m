...
alpha = 1;
Re = 1000;
Wi_batches= [12,13;14,15;16,17;18,19];



num_batches = size(Wi_batches,1);
batch_size=size(Wi_batches,2);
Re_critical_all = zeros(num_batches, batch_size);
sensits_all = zeros(num_batches, batch_size);
alpha_critical_all = zeros(num_batches, batch_size);
...
N_local = N;
parfor ii = 1:num_batches
   Wi_batch = Wi_batches(ii,:);
   Re_critical = zeros(1, batch_size);
   alpha_critical = zeros(1, batch_size);
   ee_ref_local = NaN;
   for j = 1:batch_size
    N_local = N;
    Wi = Wi_batch(j);
    if j==1
       ...
       alpha_local = alpha_critical(j-1)
       Re_local = Re_critical(j-1)
    end
    fprintf('In-Out iteration %d , %d : Wi = %.2f, alpha = %.3f\n', ii, j, Wi, alpha_local);
    % if Wi<5
    %     N_local=N_local;
    % elseif Wi>5 && Wi<10
    %     N_local = N_local +200;
    ...
    while abs(imag(eemax)) > 1e-9 && Re_local<50000
        fun = @(alpha_local) max_imag(Re_local, Wi, alpha_local, N_local,ee_ref_local);
        ...
        [eemax, dwda, dwdRe,ee_ref_new] = OB(Re_local, Wi,  alpha_local, N_local, ee_ref_local);
        fprintf('iteration %d : eemax : %.3f , dwda  = %.3f, dwdRe  = %.3f , ee_ref_updated = %.3f \n', flag_count, eemax, dwda,dwdRe,ee_ref_new);
        dRe = imag(eemax) / imag(dwdRe);
        relax = 1;
        ee_ref_local = ee_ref_new;
        Re_local = min(Re_local + relax * dRe, 50000);
        flag_count = flag_count +1;
    end

    Re_critical(j) = Re_local;
    alpha_critical(j) = alpha_local;
   end
   Re_critical_all(ii,:) = Re_critical;
   alpha_critical_all(ii,:) = alpha_critical;
end


toc
Wi_values = Wi_batches(:);
Re_flat = Re_critical_all(:);
alpha_flat = alpha_critical_all(:);
figure;
plot(Re_flat,Wi_values ,'d-','LineWidth', 2);
xlabel('Critical Reynolds Number (Re_{cr})');
ylabel('Weissenberg Number (Wi)');
title('Neutral Stability Curve (TDR Model)');
grid on;
output_data = [Wi_values(:), Re_flat(:), alpha_flat(:)];
writematrix(output_data, 'neutral_curve.csv');
save('neutral_curve.mat', 'Wi_values', 'Re_flat');

function [max_im, dwdalpha_imag, dwdRe] = max_imag(Re, Wi,alpha, N,ee_ref)
[eigg, dwdalphacalc, dwdRecalc] = OB(Re, Wi, alpha, N,ee_ref);
max_im = -imag(eigg);
dwdRe = imag(dwdRecalc);
dwdalpha_imag = imag(dwdalphacalc);
end

function [ee,dwdalpha, dwdRe,ee_ref_out] = OB(Re, Wi, alpha, N, ee_ref_in)
[D, y] = cheb(N);
D2 = D^2;
D3 = D^3;
D4 = D^4;
...
Z = null(B);
Q = (108+8*y.^6+12*(12*y.^6+81).^(1/2)).^(1/3);
integ = -Re*y.*(4*y.^4+2*Q.*y.^2+Q.^2)./(6*Q);
...
if isnan(ee_ref_in)
    ...
    [EV, evs] = eigs(Abalanced, Bbalanced, k, sigma, opts);
    evals = diag(evs);
    [~, idx] = max(imag(evals));
    ee_candidate = evals(idx);

     if abs(ee_candidate - ee_ref_in) > threshold
        [EV, evs] = eig(Abalanced, Bbalanced);
        evals =evs;
        eeOB = diag(evals);
        ix = real(eeOB)>=-0.5& real(eeOB)<=1.5;
        evals = eeOB(ix);
